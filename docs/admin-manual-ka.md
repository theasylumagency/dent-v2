...
**მარკეტინგი**
12. ლენდინგ გვერდები
13. Google-ის ტექსტები
14. საკვანძო სიტყვები
15. სტატისტიკა

**ადმინისტრირება**
16. პარამეტრები
17. პრობლემების სწრაფი მოგვარება
...
>
> **გამონაკლისი — ლენდინგ გვერდები.** იქ სავალდებულოობა მხოლოდ ქართულს ეხება, სპეციალურად. იხილე თავი 12.
...
სამ ველს აქვს ფორმატირების ზოლი: **სტატიის ტექსტი** (თავი 5), **ექიმის ბიოგრაფია** (თავი 7) და **აპარატის ტექსტი** (თავი 9). დანარჩენი ველები მარტივი ტექსტია.
...
> **ეს არ არის ის ფოსტა, სადაც ჩაწერის შეტყობინებები მოდის.** ეს საჯარო მისამართია, რომელსაც პაციენტი ხედავს. შეტყობინებების მისამართი ცალკე პარამეტრია — იხილე თავი 16.
...
**მენიუ:** საიტის შიგთავსი → კლინიკური შემთხვევები

მკურნალობამდე და მკურნალობის შემდეგ ფოტოები. ყოველი ჩანაწერი გვერდზე `/ka/cases` ერთ ბლოკად ჩნდება, თავისი ღუზით — `/ka/cases#zeda-rigis-restavratsia`.

მთავარ გვერდზე ეს ფოტოები **არ ჩანს.** იქ მხოლოდ კარია — ოთხი წყვილი სიტყვა და ბმული ამ გვერდზე. ეს განზრახაა: მკურნალობის ფოტოს ის ხედავს, ვინც მის სანახავად თავად შემოვიდა.

## თანხმობა — ერთადერთი, რაც გამოქვეყნებას აკავებს

გვერდით სვეტში ორი ჩექბოქსია და მათ შორის რიგითობას აქვს მნიშვნელობა:

| რიგით | ჩექბოქსი |
|---|---|
| ჯერ | **პაციენტის თანხმობა მიღებულია** |
| მერე | **საიტზე ჩანს** |

**„საიტზე ჩანს“ ვერ ჩაირთვება, სანამ თანხმობა მონიშნული არ არის.** შენახვის მცდელობა შეცდომას დააბრუნებს — სწორედ იმ უჯრაზე, რომელიც ახლა მონიშნე.

> **მონიშნე მხოლოდ მაშინ, თუ პაციენტისგან წერილობითი თანხმობა გაქვს.** ეს ფორმალობა არ არის: პაციენტის პირის ღრუს ფოტო მისი პერსონალური მონაცემია, და ერთხელ გამოქვეყნებულის უკან წაღება პრაქტიკულად შეუძლებელია. თუ ეჭვი გაქვს — არ მონიშნო.

სწორედ ამიტომ ამ ფორმაში **პაციენტის სახელის, ასაკის ან დიაგნოზის ველი საერთოდ არ არსებობს.** ქვეყნდება მკურნალობა და ორი ფოტო — მეტი არაფერი.

## გვერდითი სვეტი

| ველი | |
|---|---|
| **პაციენტის თანხმობა მიღებულია** | გამორთულია ნაგულისხმევად |
| **საიტზე ჩანს** | თანხმობის გარეშე არ ჩაირთვება |
| **რიგითობა** | რაც უფრო მცირე რიცხვია, მით ზემოთ დგას |
| **მისამართი (slug)** | ავტომატურად ივსება სათაურიდან |

**არსებული მისამართი არასდროს გადაიწერება** — იგივე წესი, რაც ექიმებთან (თავი 7). ის ღუზაა და უკვე გაზიარებულ ბმულში შეიძლება იჯდეს.
...
| **მიმართულება** | ✓ | — |
| **ფოტო — მკურნალობამდე** | ✓ | — |
| **ფოტო — მკურნალობის შემდეგ** | ✓ | — |
| **მოკლე აღწერა** | ✓ | ✓ |
| **ხანგრძლივობა** | — | ✓ |
| **ექიმი** | — | — |
| **დეტალური აღწერა** | — | ✓ |

**სათაური** მოკლედ ამბობს, რა გაკეთდა — „ზედა რიგის ესთეტიკური რესტავრაცია“. პაციენტის სახელი აქ არ იწერება.

**მიმართულება** იგივე ხუთია, რაც სერვისებში (თავი 6). ფოტოების თავზე პატარა წარწერად ჩანს.

**ორი ფოტო** ერთმანეთის გვერდით დგება — ორივე ერთდროულად ჩანს. გადასაწევი სლაიდერი განზრახ არ გვაქვს: ის ყოველ მომენტში ნახევარს მალავს, გვერდი კი მთლიანად იმაზეა აგებული, რომ დაიჯერონ.

> **ერთი კუთხე, ერთი განათება.** ორივე ფოტო ერთნაირად უნდა იყოს გადაღებული. სხვადასხვა კუთხით ან სხვადასხვა შუქზე გადაღებული წყვილი შედეგს **უფრო ნაკლებ დამაჯერებელს** ხდის, ვიდრე ის სინამდვილეშია — მნახველი განსხვავებას კამერას მიაწერს და არა მკურნალობას.

**მოკლე აღწერა** — ერთი-ორი წინადადება: რა იყო საწყისი მდგომარეობა და რა შეიცვალა. ეს ტექსტი ფოტოების ზემოთ დგას.

**ხანგრძლივობა** ისე დაწერე, როგორც პაციენტს ეტყოდი: „4 ვიზიტი“, „3 თვე“. ცარიელი დატოვე და საერთოდ არ გამოჩნდება.

**ექიმი** ექიმების სიიდან აირჩევა (თავი 7). ცარიელი დატოვე და ექიმის სახელი არ გამოჩნდება.

**დეტალური აღწერა** დაკეცილ ბლოკშია, რადგან უმეტეს შემთხვევაში საჭირო არ არის. ველს ფორმატირების ზოლი აქვს — იხილე **თავი 2 → ტექსტის რედაქტორი**.

## ფოტოები

ორივე ფოტო „ფოტოებიდან“ აირჩევა ან იქვე აიტვირთება — იხილე თავი 11.

> **თუ ფოტო „ფოტოებში“ წაიშალა, შემთხვევა გვერდიდან ქრება.** ნახევარი „მდე/შემდეგ“ არ არის შემთხვევის პატარა ვერსია — ეს განაცხადია, რომელსაც მტკიცებულება მოაკლდა, ამიტომ გვერდი მას საერთოდ არ აჩვენებს. ჩანაწერი ადგილზეა; ფოტო დააბრუნე და ბლოკიც დაბრუნდება.

## Google-ის ტექსტი

ამ ჩანაწერს Google-ის ველები **არ აქვს** — და ეს განზრახაა: შემთხვევას თავისი ცალკე მისამართი არ აქვს, ღუზაა ერთ გვერდზე, ამიტომ სათაურს, რომელსაც Google აჩვენებდა, წასასვლელი ადგილი არ ექნებოდა. თავად გვერდის სათაური და აღწერა „Google-ის ტექსტებში“ იცვლება — იხილე თავი 13.

## სად ჩანს

| სად | რა ჩანს |
|---|---|
| `/ka/cases` | სრული ბლოკი — ორი ფოტო, ტექსტი, ხანგრძლივობა, ექიმი |
| მთავარი გვერდი | არაფერი; მხოლოდ ბმული ამ გვერდზე |
| ჰედერის მენიუ | „ისტორიები“ |
| საიტმაპი | გვერდი |

> **სანამ ერთი შემთხვევაც არ არის გამოქვეყნებული**, გვერდი მაინც არსებობს და წერს, რომ შემთხვევები მზადდება. ის არ „ტყდება“ — უბრალოდ ცარიელია.

> **ამჟამად გვერდი გამორთულია.** სანამ კლინიკა პირველ შემთხვევებს არ მოგვაწვდის, „ისტორიები“ არც მენიუშია, არც მთავარ გვერდზე და არც საიტმაპში. ეს ერთი გადამრთველია კოდში (`routeReady.cases`) — შემთხვევების შეყვანის შემდეგ ერთი სიტყვა ჩაირთვება და სამივე ადგილი ერთდროულად ბრუნდება. **ამ თავში აღწერილი ფორმა კი უკვე მუშაობს** — შემთხვევების შეყვანა ახლაც შეგიძლია.
...
| ლენდინგი | თავად ლენდინგში, თაბი „SEO“ (თავი 12) | თითოეულს თავისი მისამართი აქვს |
...
> **ყველა საკვანძო სიტყვა ერთად რომ ნახო** — მენიუს ბოლოში, „სახელმძღვანელოს“ ზემოთ, არის **„საკვანძო სიტყვები“**. იქ ერთ სიაშია ყველა გვერდი, სამივე ენაზე. იხილე თავი 14.
...
**1. სწორ ადგილას შეავსე?** სტატიას, ექიმსა და ლენდინგს Google-ის ტექსტი **თავად ჩანაწერშია**, არა „Google-ის ტექსტების“ ეკრანზე. იხილე თავი 13-ის ცხრილი.
